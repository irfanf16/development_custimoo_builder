<template>
  <div class="customization-text-area">
    <div class="px-3 pt-3" v-for="(product_custom_text, customTextIndex) in product_custom_texts"
      :key="`parent-${selectedProductId+customTextIndex}`">

      <h2 :key="`header-${selectedProductId+customTextIndex}`"
        class="fw-bold mb-2 fz-18 d-flex align-items-center justify-content-between">
        ...
        <template v-if="product_custom_text.manually_added">
          <b-button class="ml-1 light" size="sm" style="min-width: unset; line-height: normal" variant="dark"
            :key="`custom_text_${customTextIndex}_removed_btn`" @click="removeCustomText(customTextIndex)">
            <b-icon-x />
          </b-button>
        </template>
      </h2>
      ...
      <div :key="`main-${selectedProductId+customTextIndex}`" class="d-flex">
        <b-form-input :ref="`text-customize-${customTextIndex}`" :key="`text-${selectedProductId+customTextIndex}`"
          class="mb-2 mr-sm-2 mb-sm-0" placeholder="Type Here" :value="product_custom_text.value"
          @keyup.enter="handleTextEnterEvent($event, product_custom_text, customTextIndex)"
      ...
      <div class="fs-2 text-muted">
        Font used: <span class="text-body"
          :style="{fontFamily: product_custom_text.font_family}">{{product_custom_text.font_family}}</span>
      </div>
      <div>
      ...
      <b-collapse accordion="my-accordion" v-model="text_accordion[customTextIndex]" :visible="false"
        ...
        <div>
          <div class="customization-tabs show_hide_text">
            <b-tabs content-class="mt-3" v-model="active_jersey_part[customTextIndex]"
              :key="`text_tab_${customTextIndex}`" class="sportswear-parts" align="center"
              @input="resetCustomTextColorIndex(product_custom_text)">
              <template v-for="(product_custom_text_item, productCustomTextItemIndex) in product_custom_text.items">
                <b-tab
                  :key="`custom_${product_custom_text.type}_${customTextIndex}_children_${productCustomTextItemIndex}`">
                  <!-- Tabs title slot -->
                  <template #title>
                    <span @click="($event)=>$event.stopPropagation()">
                      <b-form-checkbox v-model="product_custom_text.items[productCustomTextItemIndex].selected"
                        :name="`custom_text_child_${productCustomTextItemIndex}_checkbox`"
                        :key="`custom_text_child_${productCustomTextItemIndex}_checkbox`"
                        @change="handleCustomTextCheckboxChange($event, customTextIndex, productCustomTextItemIndex)">
                      </b-form-checkbox>
                    </span>
                        ...
                        <b-tab :key="`select_color_type_${selectColorTypeIndex}`" @click="handleTextOutline(customTextIndex, productCustomTextItemIndex, selectColorTypeIndex)">
                          <template #title>
                            <div class="color-circle" :style="{ background: selectColorTypeIndex == 0 ?
                             ...
                             product_custom_text_item.outline_color }"></div>
                            {{ select_color_type }}
                          </template>
                            ...
                            <div class="outline-slider-area d-flex justify-content-between pt-2">
                              <template v-if="product_custom_text_item.outline_enabled">
                                <div class="mr-sm-2 mb-sm-0" v-show="product_custom_text_item.color_tab_index == 1">
                                  <label class="mt-1"
                                    :for="`custom_text_${customTextIndex}_child_${productCustomTextItemIndex}_outline`">
                                    Outline Width:
                                    <span class="font-weight-bolder">{{ product_custom_text_item.outline_width
                                      }}px</span>
                                  </label>
                              ...
                              <div>
                                <label
                                  :for="`custom_text_${customTextIndex}_child_${productCustomTextItemIndex}_placement`">Placement</label>
                                <b-form-select
                                  :style="{ fontSize: '14px', height: '35px', outline: 'none', boxShadow: 'none' }"
                                  :value="product_custom_text_item.placement" :options="['Front', 'Back']"
                                  :name="`custom_text_${customTextIndex}_child_${productCustomTextItemIndex}_placement`"
                                  :key="`custom_text_${customTextIndex}_child_${productCustomTextItemIndex}_placement`"
                                  @change="handleCustomTextPlacementUpdate($event, customTextIndex, productCustomTextItemIndex)"></b-form-select>
                              </div>
                            </div>

                            <b-tabs>
                              <b-tab v-for="(product_color, productColorIndex) in productColors"
                                :key="`product_color_${productColorIndex}_${product_color.type}_type`">
                                <template #title>
                                  {{product_color.name | capitalize | removeExt}}
                                </template>
                                <div class="color-holder" @wheel="bindScroll" @scroll="bindScroll"
                                  @touchmove="bindScroll">
                                  <div class="color-container">
                                    <div class="color-box" v-for="(color, colorIndex) in product_color.color_text"
                                      :style="{background: color.value}"
                                      :key="`product_color_${productColorIndex}_${product_color.type}_type_color_${colorIndex}`"
                                      :title="color.name"
                                      @click="customTextColorUpdated(customTextIndex, productCustomTextItemIndex, color, select_color_type)">
                                    </div>
                                  </div>
                                </div>
                              </b-tab>
                                  ...
                                  <div class="color-container">
                                    <template v-for="(color, colorIndex) in logoColorsInfo">
                                      <div class="color-box" :style="{background: color.hex}" v-if="color.hex"
                                        ...
                                        @click="customTextColorUpdated(customTextIndex, productCustomTextItemIndex, {name: color.name, value: color.hex, position: '1'}, select_color_type)">
                                      </div>
                                    </template>
                                  </div>
                                </div>
                              ...
                              <b-tab v-if="lockerroomColors && lockerroomColors.length && isCustomerAuthenticated">
                                ...
                                <div class="color-holder" @wheel="bindScroll" @scroll="bindScroll"
                                  ...
                                  <div class="d-flex align-items-center overflow-auto theme-scroll-h gap-1 pb-2">
                                    <b-button size="sm" class="btn-locker-folder" variant="secondary"
                                      :class="{'active': folder_i == activeFolderIndex}"
                                      @click="setActiveFolderIndex(activeLockerIndex, folder_i)"
                                      v-for="(folder, folder_i) in lockerroomColors[activeLockerIndex].folders"
                                      :key="`folder_${activeLockerIndex}${folder_i}`">
                                      {{folder.folder_name}}
                                    </b-button>
                                  </div>
                                  <div class="color-container mt-1">
                                    <template
                                      v-for="(color, colorIndex) in JSON.parse(lockerroomColors[activeLockerIndex].folders[activeFolderIndex].color)">
                                      <div class="color-box" :style="{background: color.value}" v-if="color.value"
                                        :key="`text_color_${colorIndex}${color.name}`" :title="color.name"
                                        @click="customTextColorUpdated(customTextIndex, productCustomTextItemIndex, {...color, position: '1'}, select_color_type)">
                                      </div>
                                    </template>
                                  </div>
                                </div>
                              ...
                              <b-tab v-if="selectedProduct.is_custom_color_allowed">
                                ...
                                <div class="color-holder" @wheel="bindScroll" @scroll="bindScroll"
                                  ...
                                  <div class="color-container">
                                      ...
                                      <div class="d-flex align-items-start">
                                        ...
                                        <div class="p-2" v-if="getColorType !== 'product_color'">
                                          <div v-if="selectColorTypeIndex==0">
                                            <b-form-input @focusin="($event)=>$event.target.select()"
                                              :value="product_custom_text_item.color_pantone"
                                              class="mb-2 mr-sm-2 mb-sm-0" :placeholder="place_holder"
                                              @input="changePantoneColor($event, customTextIndex, productCustomTextItemIndex, 'Fill Color')"></b-form-input>
                                          </div>
                                          ...
                                          <div v-else>
                                            <b-form-input @focusin="($event)=>$event.target.select()"
                                              ...
                                              class="mb-2 mr-sm-2 mb-sm-0" :placeholder="place_holder"
                                              @input="changePantoneColor($event, customTextIndex, productCustomTextItemIndex, 'Outline Color')"></b-form-input>
                                          </div>
                                    ...
                                    <color-picker
                                      @changeColor="changeColor($event, customTextIndex, productCustomTextItemIndex, select_color_type)"
                                      theme="light" :colors-default="[]"
                                      :key="selectColorTypeIndex == 0 ? product_custom_text_item.color : product_custom_text_item.outline_color"
                                      :color="selectColorTypeIndex == 0 ? product_custom_text_item.color : product_custom_text_item.outline_color"
                                      :sucker-hide="true"
                                      :ref="`text-color-picker${customTextIndex}${productCustomTextItemIndex}`" />
                                  </div>
                                </div>
                              </b-tab>
