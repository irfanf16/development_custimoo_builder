<template>
  <div>
    <div class="position-relative">
      ...
      <div v-if="collection_logo? collection_logo.path : false" class="position-relative">
        ...
        <img v-if="recent_logo_preview !==''" :src="`${storage_url}${recent_logo_preview}`" :alt="collection_logo? collection_logo.name : false" class="border"
...
<!--        <img v-else-if="!collection_logo? collection_logo.fileuploading : false" :src="`${storage_url}${collection_logo? collection_logo.path : false}`" :alt="collection_logo? collection_logo.name : false" class="border"-->
<!--             style="max-height: 130px; border-color: #ccc; background-color: #ddd; margin-top: 20px; object-fit: contain; padding: 10px; width: 100%; border-radius: 7px" />-->
        <img v-else :src="`${collection_logo? collection_logo.path : false}`" :alt="collection_logo? collection_logo.name : false" class="border"
             style="max-height: 130px; border-color: #ccc; background-color: #ddd; margin-top: 20px; object-fit: contain; padding: 10px; width: 100%; border-radius: 7px">
      </div>
      <div style="padding-bottom: 10px" class="upload-logo-opener" v-else>
        <div class="btn btn-secondary modal-handler">
          <div class="upload-box position-relative" :class="{'pulse-animation': true}"
            ...
            <div class="uploaded-logo-holder" v-if="collection_logo? collection_logo.path : false">
              <img :src="`${storage_url}${collection_logo? collection_logo.path : false}`"/>
            </div>
            <div v-else>
    ...
    <div class="mt-3">
      <b-input placeholder="Logo URL"></b-input>
    </div>
    <div class="text-right cursor-pointer mt-2" @click="toggleRecentLogos">Recent Logos <b-icon-plus v-if="!show_recent" /> <b-icon-dash v-else /> </div>
...
export default class ShopLogoUploader extends Mixins(ErrorMessages, ModalAction) {
  @Prop({required: false, default: ()=>({})}) collection_logo!: Record<any, any>
  ...
  public async handleLogoChange(recent_logo){
    ...
    const newLogo = {
      path : recent_logo.logo_url,
      extension: recent_logo.logo_url.split('.').pop(),
      name: recent_logo.logo_name,
      size: 0,
      is_recent_logo: true,
    }
