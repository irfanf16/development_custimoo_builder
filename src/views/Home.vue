  ...
  async mounted() {
    ...
    * */
    ...
    if(sync_id) {
      ...
      let last_sync_id = this.$store.getters.getLastSyncId;
      if(last_sync_id != sync_id) {
  ...
  public async getLockerRoomProducts(locker_index: any) {
    ...
    if (this.isCustomerAuthenticated) {
      if (this.roomWithProducts.length === 0) {
        let res = await this.$store.dispatch('GET_LOCKER_PRODUCTS', 'fetch_all=true')
        if (res == true) {
          if (locker_index) {
     ...
     this.$store.commit('SET_DURING_RESET', true)
        ...
        if(edit_info_obj.type == 'cart_product' || edit_info_obj.type == 'order_product') {
          ...
          if(edit_info_obj.type == 'order_product') {
            ...
            product_id = edit_info_obj.cart_product_info.cart_item_product.product_id
          }
