<template>
      ...
      <div class="d-flex align-items-center justify-content-between gap-1 py-4 px-3 m-auto">
        ...
        <div class="sample-card">
          ...
          <div class="sample-images">
            <div class="img-block">
              <img :src="samples[currentSampleIndex].front" alt="Front"
            ...
            <div class="img-block">
              <img :src="samples[currentSampleIndex].back" alt="Back" 
               style="cursor:pointer"
  ...
  <button
    ...
    :class="{ active: sampleStatus[currentSampleIndex] === 'rejected' }"
  ...
  <button
    ...
    :class="{ active: sampleStatus[currentSampleIndex] === 'accepted' }"
...
export default class ThirdPartyFeedback extends Mixins(ErrorMessages) {
  samples: Sample[] = [];
  ...
  async mounted() {
    ...
    try {
      const response = await http.get(
        `admin/get-third-party-design-samples/${order_item_id}`,
        {
          params: {
            auth_token
          }
...
this.sampleFeedback = Array(this.samples.length).fill('');
  ...
  public async submitAllSamples() {
    ...
    try {
      await http.post('admin/approve-reject-third-party-samples', payload);
